...
var xorAfterQueries = function (nums, queries) {
  ...
  for (const [l, r, k, v] of queries) {
    if (k < B) {
      if (diff[k] === null) {
        diff[k] = new Array(n + k).fill(1n)
      }
      diff[k][l] = (diff[k][l] * BigInt(v)) % MOD
      const rem = (r - l) % k
      const rPos = r - rem + k
      const invV = modInverse(BigInt(v), MOD)
  ...
  for (let k = 0; k < B; k++) {
    ...
    for (let start = 0; start < k; start++) {
      ...
      for (let i = start; i < n; i += k) {
        mul = (mul * d[i]) % MOD
        nums[i] = Number((BigInt(nums[i]) * mul) % MOD)
      }
    }
  }

  return nums.reduce((xorSum, num) => xorSum ^ num, 0)
}
