var maxPathScore = function (grid, k) {
  ...
  const memo = new Array(m * n * (k + 1)).fill(null)
  const dfs = (i, j, r) => {
    if (i >= m || j >= n) return -Infinity
    const c = grid[i][j] > 0 ? 1 : 0
    if (r < c) return -Infinity
    const idx = (i * n + j) * (k + 1) + r
    if (memo[idx] !== null) return memo[idx]
    if (i === m - 1 && j === n - 1) return (memo[idx] = grid[i][j])
    return (memo[idx] = grid[i][j] + Math.max(dfs(i + 1, j, r - c), dfs(i, j + 1, r - c)))
  }
  const ans = dfs(0, 0, k)
