...
var minMoves = function (nums, limit) {
  ...
  for (let i = 0; i < Math.floor(n / 2); i++) {
    const x = nums[i]
    const y = nums[n - 1 - i]
    const l = Math.min(x, y) + 1
    const r = Math.max(x, y) + limit
...
const __lcExamples = [
  {
    args: [[1, 2, 4, 3], 4],
    expected: 1,
    comment: '// 输入：nums = [1,2,4,3], limit = 4  输出：1',
  },
  {
    args: [[1, 2, 2, 1], 2],
    expected: 2,
    comment: '// 输入：nums = [1,2,2,1], limit = 2  输出：2',
  },
  {
    args: [[1, 2, 1, 2], 2],
    expected: 0,
    comment: '// 输入：nums = [1,2,1,2], limit = 2  输出：0',
  },
]

__lcRunExamples(minMoves, __lcExamples)
