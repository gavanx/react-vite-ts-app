var numberOfStableArrays = function (zero, one, limit) {
  ...
  const dfs = (i, j, k) => {
    ...
    if (k === 0) {
      const ret =
        (dfs(i - 1, j, 0) + dfs(i - 1, j, 1) - (i > limit ? dfs(i - limit - 1, j, 1) : 0) + MOD) % MOD
      cache.set(key, ret)
      return ret
    } else {
      const ret =
        (dfs(i, j - 1, 0) + dfs(i, j - 1, 1) - (j > limit ? dfs(i, j - limit - 1, 0) : 0) + MOD) % MOD
      cache.set(key, ret)
      return ret
    }
  }
...
console.log(numberOfStableArrays(39, 20, 18))
