...
var specialNodes = function (n, edges, x, y, z) {
  ...
  // Optimized BFS using array-based queue with head pointer
  const bfs = (start) => {
    const dist = Array(n).fill(-1)
    dist[start] = 0

    // Use array as queue with head pointer for O(1) dequeue
    const queue = [start]
    let head = 0
  ...
  // Only compute distances from the three special nodes
  const distX = bfs(x)
  const distY = bfs(y)
  const distZ = bfs(z)
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Skip if any node is unreachable
    if (dx === -1 || dy === -1 || dz === -1) continue

    // Sort distances to get a, b, c where a ≤ b ≤ c
