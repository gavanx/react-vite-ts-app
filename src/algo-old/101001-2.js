var minDistinctFreqPair = function (nums) {
  ...
  return Array.from(map2.values()).sort().slice(0, 2)
}
...
const __lcExamples = [
  { args: [[1, 1, 2, 2, 3, 4]], expected: [1, 3], comment: "// 输入：nums = [1,1,2,2,3,4]  输出：[1,3]" },
  { args: [[1, 5]], expected: [-1, -1], comment: "// 输入：nums = [1,5]  输出：[-1,-1]" },
  { args: [[7]], expected: [-1, -1], comment: "// 输入：nums = [7]  输出：[-1,-1]" },
];

__lcRunExamples(minDistinctFreqPair, __lcExamples);