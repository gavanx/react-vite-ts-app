const MX = 100001
const divisors = new Array(MX).fill(0).map(() => new Array())
for (let i = 1; i < MX; i++) {
  for (j = i; j < MX; j += i) {
    divisors[j].push(i)
  }
...
const __lcExamples = [
  { args: [[3, 6, 2]], expected: 7, comment: '// 输入：nums = [3,6,2]  输出：7' },
  { args: [[4, 2, 8, 3]], expected: 9, comment: '// 输入：nums = [4,2,8,3]  输出：9' },
  { args: [[7, 5, 9]], expected: 21, comment: '// 输入：nums = [7,5,9]  输出：21' },
]

__lcRunExamples(minArraySum, __lcExamples)
